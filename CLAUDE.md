...
- **문구를 고친 자리의 부정 단언은 새 문구가 아니라 "그 각주가 아예 없을 것"을 검사한다.** `not.toContain("<새 문구>")`는 **옛 문구가 뜨는 상태를 통과시킨다** — 고친 조건이 되살아나도 초록이라 가드가 아니다. 두 문구의 공통 조각으로 막는다(`not.toContain("50종목")`). **새로 붙이거나 고친 가드는 실패 주입으로 확인한다** — 고친 소스를 되돌리고(`git stash push -- src`) 새 테스트가 전부 빨개지는지 본다. v0.51.1에서 7개 중 6개만 빨개져 가짜 가드 하나를 잡았다. **가짜 가드는 통과 화면에서 안 보이고 주입에서만 드러난다.**
