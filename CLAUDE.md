...
키움증권 REST API를 **읽기 전용**으로 노출하는 MCP 서버. TypeScript(ESM, NodeNext) + `@modelcontextprotocol/sdk` + zod. 기본 37개 tool, ISA 세금 tool 1개는 opt-in.
...
- TR 응답 fixture는 **반드시** `types.ts`의 스키마로 `.parse()`해서 넣는다 — 현재 TR 응답을 다루는 17개 파일 전부가 예외 없이 이렇게 한다. 스키마와 fixture가 같이 어긋나는 걸 막는 장치다. (순수 유틸·전송·OAuth 테스트는 애초에 TR 응답을 다루지 않으므로 해당 없음.)
