...
- **기준일 기본값을 오늘로 두기 전에 그 TR이 당일 행을 주는지 확인한다.** 안 주는 TR이 있다 — ka90012는 당일 집계를 장중에 내주지 않아 `dt=오늘`이면 rc=0에 0행이고 전 거래일은 50행이다(2026-08-14 실측). 기본값이 오늘이면 **날짜를 지정하지 않은 호출이 통째로 빈손**이 되는데, 에러가 아니라서 "그런 날인가 보다"로 읽힌다. 처방은 ① 날짜를 **사용자가 고르지 않았을 때만** 한 번 물러서고 ② 물러섰다는 사실을 `※`로 밝히고(제목의 날짜만으로는 못 알아챈다) ③ 빈 결과 힌트의 원인을 휴장일이 아니라 **당일 미집계부터** 대는 것이다. 물러서는 호출도 `api.ts` 안에서 하고 간격은 `PAGE_INTERVAL_MS`를 쓴다.
