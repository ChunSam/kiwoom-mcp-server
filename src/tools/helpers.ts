 ...
 *
 * 거래원(ka10002)은 키움이 KRX 기준으로만 주므로 이 각주를 붙이지 않는다.
 */
...
/**
 * 통합으로 조회할 수 없어 KRX 기준으로 남은 tool의 각주 — 이제 ka10002(거래원) 하나뿐이다.
 * 이 TR은 `_AL` 접미사를 무시하고 KRX 값만 준다(실측 2026-08-03).
 *
 * 호가는 v0.37.0에서 빠졌다 — ka10004도 같은 제약이었지만, ka10007(시세표성정보)이 통합
 * 호가를 주는 걸 실측하고 get_orderbook을 그쪽으로 옮겼다.
 *
 * 이 각주가 없으면 같은 종목에 대해 get_stock_price는 통합 거래량을, 이 tool은 KRX 수량을
 * 보여주는 비대칭이 아무 설명 없이 노출된다 — 사용자가 두 숫자를 합산하거나 비교하면
 * 틀린 결론에 닿는다.
 */
export const KRX_ONLY_NOTE =
  "※ 거래원은 키움이 통합 시세를 제공하지 않아 **KRX 기준**입니다 — " +
