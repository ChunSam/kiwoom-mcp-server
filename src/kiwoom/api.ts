...
export async function fetchLendingBalanceRank(
  ...
  // 장중 내내 빈손이 된다.
  //
  // **언제 열리는지도 찍었다**(2026-08-19, 9표본): 16:38~19:07은 계속 0행이고 20:07에 50행이
  // 됐다 — 열림은 **19:07~20:07 사이**다(전날 8/18 20:52 열림과 일관). 열린 값은 **확정치**라
  // 잠정 각주가 필요 없다 — 8/18치가 다음 날 저녁까지 행·필드 완전 동일했다. 같은 시각
  // 모의·실전 18응답이 전부 일치해 이 TR은 mockapi가 실전을 그대로 미러한다.
  // 포맷터가 "저녁에 다시 부르세요"를 안내할 수 있는 근거가 이것이다.
