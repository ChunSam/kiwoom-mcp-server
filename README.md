...
예외 두 가지: **호가(`get_orderbook`)와 거래원(`get_broker_activity`)은 KRX 기준**입니다
(키움이 통합으로 주지 않습니다). **`get_after_hours`(시간외 단일가)** 역시 통합 조회가
불가능하며, NXT 거래가능 종목은 애초에 이 TR에서 조회되지 않습니다.
